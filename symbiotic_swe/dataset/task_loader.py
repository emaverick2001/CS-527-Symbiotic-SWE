...
def build_task_subsets(
    ...
    usable = sort_candidates([candidate for candidate in candidates if not candidate.exclusion_reasons])
    skipped = sort_candidates([candidate for candidate in candidates if candidate.exclusion_reasons])

    smoke_pool = [candidate for candidate in usable if candidate.include_for_smoke]
    smoke_tasks = balanced_take(smoke_pool, smoke_count, preferred_repos)
    used_ids = {candidate.raw.instance_id for candidate in smoke_tasks}
    ...
    dev_tasks = balanced_take(remaining, dev_count, preferred_repos)
    used_ids.update(candidate.raw.instance_id for candidate in dev_tasks)

    final_pool = [
        candidate
        for candidate in remaining
        if candidate.raw.instance_id not in used_ids
    ]
    final_eval_tasks = balanced_take(final_pool, final_eval_count, preferred_repos)
...
def export_ranked_raw_subsets(
    ...
    smoke_candidates, core_candidates, _final_candidates, _skipped = loader.select_subsets(candidates)
