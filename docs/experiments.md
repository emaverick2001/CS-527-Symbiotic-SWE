...
- That remains the benchmark-facing success criterion, but this paper reports **Real-Test Success Rate** as the primary test-facing metric:

  $$
  \text{RealTestSuccessRate} =
  \frac{\#\text{tasks where the patch applies, FAIL\_TO\_PASS passes, and PASS\_TO\_PASS passes when available}}
       {\#\text{tasks}}
  $$

- We also measure the **Logical Correctness Rate**, which is stricter than test-passing. A patch is counted as logically correct only when real tests pass and symbolic verification on the final checked patch returns `UNSAT`:

  $$
  \text{LogicalCorrectnessRate} =
  \frac{\#\text{tasks where real tests pass and final solver status is UNSAT}}
       {\#\text{tasks}}
  $$

- This avoids treating tasks where the solver did not run as logically verified. We therefore also report **Solver Coverage Rate**:

  $$
  \text{SolverCoverageRate} =
  \frac{\#\text{tasks with at least one solver result}}
       {\#\text{tasks}}
  $$
...
- The second is **Tokens per Success**, [ \text{TPS} = \frac{\text{Total LLM Tokens Used}}{\#\text{Real-Test Successful Tasks}}, ] which measures how expensive successful repair is in model usage terms. The third is **Solver Overhead**, [ \text{SolverOverhead} = \frac{\text{Symbolic Verification Time}}{\text{Total Runtime}}, ] which separates the extra formal-analysis cost from the rest of the loop. The fourth is total wall-clock runtime. These are paired with behavioral diagnostics—counterexample detection rate, critique utility, repeated failure rate, and a failure-mode breakdown—so that the paper can explain not only whether the method helps, but how it changes the agent’s search behavior.
##### **1. Effectiveness (Primary Metrics)**
- These are your **most important metrics**.
---
1. ==**Real-Test Success Rate**==
	- What it measures
		- % of tasks successfully solved according to real benchmark tests
		- Your main benchmark comparison metric
	- Formula
		- RealTestSuccessRate = (# tasks where patch applies, FAIL_TO_PASS passes, and PASS_TO_PASS passes when available) / (# tasks)
...
2. ==**Logical Correctness Rate**==
	...
	- **Formula**
		- LogicalCorrectnessRate = (# tasks where real tests pass AND final solver status is `UNSAT`) / (total tasks)
		- Also report SolverCoverageRate = (# tasks with at least one solver result) / (total tasks)
	- **Expected Direction of Change**
...
1. ==**Tokens per Success**==
	...
	- Formula
		- TokensPerSuccess = TotalTokensUsed / \#Real-Test Successful Tasks
	- Expected Direction of Change
		- ↓ significantly
	- Why it matters
...
- The inclusion rule is simple and implementation-oriented: keep tasks whose gold fixes primarily adjust conditionals, boolean expressions, comparison operators, arithmetic guards, or small-scope return logic; exclude tasks dominated by dependency changes, large refactors, file I/O, configuration edits, or architectural redesign. This restriction is scientifically motivated because the proposed verifier is local and predicate-focused, and practically motivated because whole-repository symbolic reasoning is out of scope. As a fast smoke-test bed, the study also uses the Python repair tasks from HumanEvalPack’s HumanEvalFix benchmark, which extends HumanEval into code repair and explicitly distinguishes test-only and docstring-only repair variants.
##### SWE-bench Verified
###### **Scale**
- 500 + issues
###### **Structure & Format**
...
- As a fast smoke-test bed, the study also uses the Python repair tasks from HumanEvalPack’s HumanEvalFix benchmark, which extends HumanEval into code repair and explicitly distinguishes test-only and docstring-only repair variants.
###### **Scale**
###### **Structure & Format
#### **Baselines**
...
- This makes the study comparable in spirit to the “bash-only” SWE-bench Verified setup, which intentionally evaluates a simple ReAct-style loop rather than a complex scaffolding stack. #TODO
...
---
1. Neural + slicing, disabled solver feedback 
	- tests whether narrowing the analysis region alone helps.
2. Neural + solver, no critique
	- testing whether the benefit comes from the existence of a formal verifier or from the feedback loop itself
#### **Case Studies**
...
- Where the sample size is small, the paper will emphasize paired deltas and bootstrap confidence intervals rather than overclaiming statistical significance. #TODO
